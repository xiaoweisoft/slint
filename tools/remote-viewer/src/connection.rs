        ...
        while let Some(msg) = receiver.next().await {
            match msg {
                ...
                Ok(Message::Binary(bin)) => {
                    ...
                    match postcard::from_bytes::<preview_protocol::LspToPreviewMessage>(&bin) {
                        Ok(message) => {
                            tracing::debug!("Received message {message:?}");
                            // Process the data
                            match message {
                                preview_protocol::LspToPreviewMessage::InvalidateContents {
                                    url,
                                } => {
                                    file_cache.remove(&url);
                                }
                                preview_protocol::LspToPreviewMessage::ForgetFile { url } => {
                                ...
                                preview_protocol::LspToPreviewMessage::SetContents {
                                    url,
                                    contents,
                                } => {
                                ...
                                preview_protocol::LspToPreviewMessage::SetConfiguration {
                                    config,
                                } => {
                                    message_handler(ConnectionMessage::SetConfiguration { config });
                                }
                                preview_protocol::LspToPreviewMessage::ShowPreview(
                                    preview_component,
                                ) => {
                                ...
                                preview_protocol::LspToPreviewMessage::Quit => {
                                    break;
                                }
                            }
                        }
