...
impl Transform {
    pub fn affine_item_transform(self: Pin<&Self>) -> crate::lengths::ItemTransform {
        let origin = self.transform_origin().to_euclid().to_vector();
        crate::lengths::ItemTransform::translation(-origin.x, -origin.y)
            .then_scale(self.transform_scale_x(), self.transform_scale_y())
            .then_rotate(euclid::Angle::degrees(self.transform_rotation()))
            .then_translate(origin)
    }

    pub fn item_transform(self: Pin<&Self>) -> crate::lengths::ItemTransform {
        let origin = self.transform_origin().to_euclid().to_vector();
        ...
        transform.then_translate(origin)
    }
...
impl Item for Transform {
    ...
    fn render(
        ...
        if self.has_projective_transform() && (*backend).supports_projective_transformations() {
            (*backend).projective_transform(self.item_transform());
        } else {
            let origin = self.transform_origin().to_euclid().to_vector();
