...
The docs use a lot of autogenerated content (mdx files, screenshots).
Build everything in one step:

```bash
cargo run --release -p slint-doc-generator
```

This generates mdx files, screenshots, installs npm dependencies,
and builds the site into `dist/`.
