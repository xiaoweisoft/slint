...
Transforms allow for rotating and scaling items around a specified origin point. The default origin point is the center of the element. Skia renderers also support projecting a flat item subtree with pitch, yaw, and perspective; rendering and pointer input use the same mapping.
...
#### transform-pitch
<SlintProperty propName="transform-pitch" typeName="angle" defaultValue='0deg'>
Rotates the element and its children around the horizontal axis. Set `transform-perspective` to a positive length to apply perspective projection.
</SlintProperty>

#### transform-yaw
<SlintProperty propName="transform-yaw" typeName="angle" defaultValue='0deg'>
Rotates the element and its children around the vertical axis. Set `transform-perspective` to a positive length to apply perspective projection.
</SlintProperty>

#### transform-translate-z
<SlintProperty propName="transform-translate-z" typeName="length" defaultValue='0px'>
Moves the element and its children along their local depth axis. Positive values move toward the camera when `transform-perspective` is positive. This property is currently supported by the Skia renderer.
</SlintProperty>

#### transform-perspective
<SlintProperty propName="transform-perspective" typeName="length" defaultValue='0px'>
Sets the camera distance for pitch and yaw projection. Zero disables perspective. This property is currently supported by the Skia renderer.
</SlintProperty>
