    ...
    for m in members {
        match m.kind {
            MemberKind::SectionHeader => {
                if m.description.starts_with("## Properties") {
                    in_properties = true;
                }
                if m.description.starts_with("## Callbacks") {
                    in_callbacks = true;
                }
                if m.description.starts_with("## Functions") {
                    in_functions = true;
                }
                writeln!(file, "{}", m.description)?;
                writeln!(file)?;
...
fn write_sub_element(
    ...
    if !child.description.is_empty() {
        writeln!(file, "{}", child.description)?;
        writeln!(file)?;
    }
